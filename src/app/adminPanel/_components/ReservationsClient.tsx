...
import { ReservationData, createReservation, deleteReservation, updateReservation, getReservations } from '../_actions/reservationActions';
...
export default function ReservationsClient({ initialReservations }: ReservationsClientProps) {
  const [reservations, setReservations] = useState<ReservationFromServer[]>(initialReservations);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [editingReservation, setEditingReservation] = useState<ReservationFromServer | null>(null);
  const [reservationToDelete, setReservationToDelete] = useState<ReservationFromServer | null>(null);
  ...
  useEffect(() => {
      ...
      // ... (logika filtrowania i sortowania)
       processedReservations.sort((a, b) => {
          ...
          const dateA = new Date(a[sortValue as keyof ReservationFromServer] as string);
          const dateB = new Date(b[sortValue as keyof ReservationFromServer] as string);
  ...
  const handleCreate = async (data: ReservationData) => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        const updatedReservations = await getReservations(sortValue, sortDesc ? 'desc' : 'asc');
  ...
  return (
    <div>
      ...
      {!showCreateForm && !editingReservation && !isDeleteModalOpen && !showFilterPanel ? ( // Nie pokazuj przycisku "Dodaj", gdy edytujesz, tworzysz, masz modal lub filtry
        <div className="mb-4">
          <button
      ...
      ) : null} {/* Nie renderuj nic, gdy edytujesz, masz modal lub filtry */}
        ...
        {/* Pokaż sekcję sortowania/filtrów tylko gdy nie edytujesz/tworzysz i nie masz otwartego modala/filtrow */}
         {!showCreateForm && !editingReservation && !isDeleteModalOpen && !showFilterPanel && (
             <div className="mb-6 p-4 border rounded shadow-lg bg-white flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    <label htmlFor="sortList" className="font-bold">Sortuj:</label>
                    <select
                        id="sortList"
                        value={sortValue}
                    ...
                    <button onClick={toggleSortDirection} className="p-2 border rounded" disabled={isPending}>
                       {sortDesc ? '▼' : '▲'}
                    </button>
                </div>
                 {/* Filtry */}
      ...
      {!showCreateForm && !editingReservation && !isDeleteModalOpen && !showFilterPanel && (
        ...
        {/* Modale i Formularze nakładające się - RENDERING POZA NORMALNYM PRZEPŁYWEM (opcjonalnie z Portalem) */}
         ...
         {isDeleteModalOpen && reservationToDelete && (
             console.log('Rendering ConfirmationModal. isDeleteModalOpen:', isDeleteModalOpen, 'reservationToDelete:', reservationToDelete),
             <ConfirmationModal
